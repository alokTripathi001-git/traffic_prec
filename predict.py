...
#         lan_aur_lat_jankari = latitude_and_longitude(origin, destination)

#         if lan_aur_lat_jankari:
#             origin_lat, origin_lon, destination_lat, destination_lon = lan_aur_lat_jankari

#             url = (
#                 f"https://maps.gomaps.pro/maps/api/directions/json"
...
#                 # distance & duration
#                 distance = f"Distance: {leg['distance']['text']}"
#                 duration = f"Duration: {leg['duration']['text']}"
#                 result = [distance, duration]        

#                 for i, step in enumerate(leg['steps']):
#                     instr = re.sub(r'<.*?>', '', step['html_instructions'])
#                     dist_m = step['distance']['value']
#                     steps.append(f"{i+1}. {instr} ({dist_m}m)")
            
#                 count = 0
#                 for s in steps:
#                     if count == 6:
#                         break
#                     if re.search(r'\b(NH|SH|National Highway)', s):
#                         major_steps.append(s[3:])
#                         count += 1

#                 # overview polyline for map
#                 overview_polyline = route['overview_polyline']['points']       
    ...
    #     steps=steps,             
    #     saved_coords=saved_coordinates        
    # )
...
#         # डेटा संग्रह
#         saved_coordinates.append({'lat': lat, 'lng': lng})
...
def traffic_prediction(): 
    ...
    if request.method=='POST':
        ...
        # url = f"https://api.tomtom.com/traffic/services/4/flowSegmentData/absolute/10/json?point={lat_and_lon[1]},{lat_and_lon[0]}&key={API_KEY}"     
        ...
        # if current_speed==0:
        #     return 0
        # if type(current_speed)==str or type(free_flow_speed)==str:
        #     return "Not available"
        # else:
        #     vehicle_count = (free_flow_speed / current_speed) * constant
        # model = joblib.load(open('models/grid_knearest.pkl', 'rb')) 
        # labelenc = joblib.load(open('models/labelenc.pkl', 'rb'))
        # weekday=labelenc.transform([weekday.title()])
        # time_hours=int(time[0:2])
        # time_minutes=int(time[3:5]) 
        # time_seconds=int(time[6:8])                                         
        # car_count,bikeCount,busCount,truckCount=int(vehicle_count//2),int(vehicle_count//6),int(vehicle_count//6),int(vehicle_count//7)   

 
        # answer=model.predict([[int(date[8:10]),weekday[0],car_count, bikeCount, busCount, truckCount,vehicle_count,time_hours,time_minutes,time_seconds]]) 
        # answer=answer[0]                  
        # print(answer)  
        # # # Output
        # if answer == 0:
